module Theme.Page.Blog exposing (viewNewsArticle, viewNewsList)

import Copy.Keys exposing (Key(..))
import Copy.Text exposing (t)
import Css exposing (Style, after, auto, batch, borderBox, borderRadius, boxSizing, calc, center, color, displayFlex, em, flexGrow, fontSize, fontStyle, fontWeight, height, important, int, italic, left, lineHeight, margin, margin2, margin4, marginBottom, marginTop, maxWidth, minus, padding, padding4, paddingLeft, pct, position, property, px, relative, rem, textAlign, width)
import Data.PlaceCal.Articles
import Helpers.TransDate as TransDate
import Helpers.TransRoutes as TransRoutes exposing (Route(..))
import Html.Styled exposing (Html, a, article, div, h3, img, li, p, section, span, text, time, ul)
import Html.Styled.Attributes exposing (alt, css, href, src)
import Skin.Global exposing (colorWhite, introTextLargeStyle, linkStyle, primaryBackgroundStyle, secondaryButtonOnLightBackgroundStyle)
                ...
                [ a
                    ...
                    , href
                        (TransRoutes.toAbsoluteUrl
                            (BlogItem (TransRoutes.stringToSlug newsItem.title))
                        )
                    ]
                    [ text newsItem.title ]
                ]
